...
openCard=function(type,id){
  ...
  const view=window.PanelAllView,h=view?.historyMeta?.(x),shown=view?.displayState?.(x)||x.status,changed=!!h?.ultima_modificacion_detectada;$('#modalContent').innerHTML=`<div class="meta"><span class="pill ${x.type}">${types[x.type]?.name}</span><span class="pill state ${statusClass(shown)}">${esc(shown)}</span>${changed?'<span class="pill state modal-updated">Actualizada</span>':''}</div><h2>${esc(x.title)}</h2><p>${esc(x.summary)}</p>${x.detail?`<div class="detail">${esc(x.detail).replace(/\n/g,'<br>')}</div>`:''}<div class="facts"><span><b>Ámbito</b>${esc(x.scope||'No indicado')}</span><span><b>Fecha</b>${esc(x.date||'No indicada')}</span><span><b>Nivel / referencia</b>${esc(x.level||'Fuente oficial')}</span></div><div class="source"><span>Fuente oficial</span><strong>${esc(x.source)}</strong>${x.sourceUrl?`<a href="${esc(x.sourceUrl)}" target="_blank" rel="noopener noreferrer">${esc(x.sourceLabel||'Abrir fuente oficial ↗')}</a>`:''}</div>`;
