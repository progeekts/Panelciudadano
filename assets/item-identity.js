...
openCard=function(type,id){
  ...
  const view=window.PanelAllView,h=view?.historyMeta?.(x),shown=view?.displayState?.(x)||x.status,changed=!!h?.ultima_modificacion_detectada;
  const changedText=changed?(()=>{const ms=Date.now()-Date.parse(h.ultima_modificacion_detectada),hrs=Math.max(0,Math.floor(ms/3600000));return hrs<1?'Actualizada hace menos de 1 h':hrs<24?`Actualizada hace ${hrs} h`:`Actualizada hace ${Math.floor(hrs/24)} d`})():'';
  const sectionName={ayudas:'Datos de la convocatoria',estafas:'Información sobre el fraude',servicios:'Estado de la incidencia',alimentacion:'Información de la alerta alimentaria',salud:'Información del aviso sanitario',meteo:'Información del aviso meteorológico'}[x.type]||'Información del aviso';const detailHtml=x.detail?`<section class="modal-info"><h3>${esc(sectionName)}</h3><div class="detail">${esc(x.detail).replace(/\\n\\n/g,'</p><p>').replace(/\\n/g,'<br>')}</div></section>`:'';
