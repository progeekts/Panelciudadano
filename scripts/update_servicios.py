...
OUT=Path('data/servicios.json');HISTORY=Path('data/historico_servicios.json');UA='PanelCiudadano/1.6 (+GitHub Pages; monitor de estado)'
...
def health_add(health,name,state,detail,scope=''):health.append({'fuente':name,'estado':state,'detalle':detail,'ambito':scope})
...
def add_autonomic_health(health):
 for name,url,scope in AUTONOMIC_PAGES:
  if name=='Ciudad Autónoma de Melilla':continue
  fetch_public(health,name,url,scope)
...
def main():
 ...
 add_age_mail(items,health);add_gencat(items,health);add_enaire(items,health);add_sepe(items,health);add_melilla(items,health);add_public_health(health);add_autonomic_health(health)
