#!/usr/bin/env python3
"""Recoge incidencias activas y conserva su ciclo de vida.

Las desapariciones solo se marcan como resueltas cuando la revisión es completa.
La fecha de resolución es la fecha en que Panel Ciudadano detecta la desaparición,
no una afirmación sobre la hora exacta en que el proveedor resolvió el problema.
"""
from __future__ import annotations
import html,json,re,urllib.request
from datetime import datetime,timezone
from pathlib import Path
OUT=Path('data/servicios.json');HISTORY=Path('data/historico_servicios.json')
UA='PanelCiudadano/1.2 (+GitHub Pages; monitor de estado)'
STATUS_SOURCES=[('GitHub','https://www.githubstatus.com/api/v2/incidents/unresolved.json','https://www.githubstatus.com/'),('Cloudflare','https://www.cloudflarestatus.com/api/v2/incidents/unresolved.json','https://www.cloudflarestatus.com/')]
...
def main():
 ...
 items=list({x['id']:x for x in items}.values());complete=not errors
 previous=load(OUT,{});previous_ids={x.get('id') for x in previous.get('items',[])};current_ids={x['id'] for x in items}
 result={'ultima_revision':now,'revision':'completa' if complete else 'parcial','fuentes_ok':ok,'fuentes_error':errors,'cambios':{'nuevas':len(current_ids-previous_ids),'desaparecidas_detectadas':len(previous_ids-current_ids) if complete else 0},'total':len(items),'items':items}
 OUT.parent.mkdir(parents=True,exist_ok=True);tmp=OUT.with_suffix('.tmp');tmp.write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');tmp.replace(OUT);update_history(items,complete,now)
 print(f"Servicios: {len(items)} activas; nuevas {result['cambios']['nuevas']}; desaparecidas {result['cambios']['desaparecidas_detectadas']}")
