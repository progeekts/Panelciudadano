...
OUT=Path('data/servicios.json');HISTORY=Path('data/historico_servicios.json');UA='PanelCiudadano/1.4 (+GitHub Pages; monitor de estado)'
STATUS_SOURCES=[
...
# Sedes públicas: se comprueba disponibilidad y solo se publica una incidencia si la propia página contiene un aviso inequívoco.
PUBLIC_PAGES=[
 ('Punto de Acceso General','https://sede.administracion.gob.es/'),
 ('Agencia Tributaria (AEAT)','https://sede.agenciatributaria.gob.es/'),
 ('Seguridad Social','https://sede.seg-social.gob.es/'),
 ('SEPE','https://sede.sepe.gob.es/'),
 ('DGT','https://sede.dgt.gob.es/'),
 ('Ministerio de Transportes','https://sede.transportes.gob.es/'),
]
...
def add_status_api(items,health,service,api,status_page,scope):
 try:p=get_json(api);health.append({'fuente':service,'estado':'ok','detalle':'API oficial de estado respondió'})
 except Exception as e:health.append({'fuente':service,'estado':'error','detalle':type(e).__name__});return
...
def fetch_public(health,name,url):
 try:text=clean(get(url));health.append({'fuente':name,'estado':'ok','detalle':'Sede oficial accesible'});return text
 except Exception as e:health.append({'fuente':name,'estado':'error','detalle':type(e).__name__});return ''
...
def add_gencat(items,health):
 name='Generalitat de Catalunya';url='https://web.gencat.cat/es/seu-electronica/informacio/avisos-i-talls-de-serveis';text=fetch_public(health,name,url)
 if not text:return
...
def main():
 ...
 add_age_mail(items,health);add_gencat(items,health);add_enaire(items,health);add_sepe(items,health);add_public_health(health)
