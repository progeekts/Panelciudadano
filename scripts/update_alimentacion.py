...
def extract(page,kind,label):
 ...
 for m in re.finditer(r'<a\b[^>]*href=["\']([^"\']*/alertas/[^"\']+)["\'][^>]*>(.*?)</a>',page,re.I|re.S):
  ...
  seen.add(href);found.append({'id':'aesan-'+hashlib.sha1(href.encode()).hexdigest()[:10],'tipo':'alimentacion','categoria':kind,'categoria_nombre':label,'titulo':title,'fecha':date,'referencia':ref_of(title),'ambito':'España','fuente':'AESAN · Red de alerta alimentaria','url':href,'verificado':True})
 return found
def main():
