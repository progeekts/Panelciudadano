...
OUT=Path('data/alimentacion.json');BASE='https://www.aesan.gob.es';UA='PanelCiudadano/1.2 (+GitHub Pages; radar ciudadano)'
...
DATE_RE=r'\d{1,2}\s+(?:enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)\s+\d{4}'
def get(url):
...
def iso_date(text):
 ...
 return f'{int(m.group(3)):04d}-{MONTHS[m.group(2).lower()]:02d}-{int(m.group(1)):02d}' if m else ''
def ref_of(title):
 m=re.search(r'Ref\.?\s*:?[ ]*(ES\s*\d{4}\s*/\s*\d+)',title,re.I);return re.sub(r'\s+','',m.group(1)).upper() if m else ''
def tidy_title(text):
 text=re.sub(r'^(?:cookie|error_outline)\s+','',text,flags=re.I);text=re.sub(r'^'+DATE_RE+r'\s*','',text,flags=re.I);text=re.sub(r'\s+Ver\s+m[aá]s\s*$','',text,flags=re.I);return text.strip()
def extract(page,kind,label):
 ...
 for m in re.finditer(r'<a\b[^>]*href=["\']([^"\']*/alertas/[^"\']+)["\'][^>]*>(.*?)</a>',page,re.I|re.S):
  ...
  seen.add(href);ref=ref_of(title);suffix=hashlib.sha1(href.encode()).hexdigest()[:10]
  found.append({'id':'aesan-'+suffix,'tipo':'alimentacion','categoria':kind,'categoria_nombre':label,'titulo':title,'fecha':date,'referencia':ref,'ambito':'España','fuente':'AESAN · Red de alerta alimentaria','url':href,'verificado':True})
 return found
def main():
 now=datetime.now(timezone.utc).isoformat();items=[];health=[]
 for kind,label,url in SOURCES:
  try:rows=extract(get(url),kind,label);health.append({'fuente':label,'estado':'ok','detectadas':len(rows)});items.extend(rows)
  except Exception as e:health.append({'fuente':label,'estado':'error','error':type(e).__name__})
 ok=[x for x in health if x['estado']=='ok']
 if not ok:print('AESAN no respondió: se conserva el fichero anterior.');return
 unique={x['url']:x for x in items};items=list(unique.values());cutoff=(datetime.now(timezone.utc)-timedelta(days=180)).date().isoformat();items=[x for x in items if x['fecha']>=cutoff];items.sort(key=lambda x:x['fecha'],reverse=True)
 result={'ultima_revision':now,'revision':'completa' if len(ok)==len(SOURCES) else 'parcial','fuente_principal':'Agencia Española de Seguridad Alimentaria y Nutrición (AESAN)','cobertura':['Alertas de interés para toda la población','Alertas para personas con alergias o intolerancias'],'nota':'Publicaciones oficiales recientes. No se infiere que una retirada siga activa: la ficha oficial determina las medidas vigentes.','salud_fuentes':health,'total':len(items),'items':items[:80]}
