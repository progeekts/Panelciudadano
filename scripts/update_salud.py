...
OUT=Path('data/salud.json');UA='PanelCiudadano/1.1 (+GitHub Pages; radar ciudadano)'
SOURCES=[('Alertas farmacéuticas','https://www.aemps.gob.es/category/informa/alertas/medicamentosusohumano-2/','farmaceutica'),('Seguridad de productos sanitarios','https://www.aemps.gob.es/comunicacion/notas-de-seguridad/notas-informativas-de-seguridad-de-productos-sanitarios/','producto_sanitario'),('Medicamentos ilegales','https://www.aemps.gob.es/category/informa/notasinformativas/medilegales-notasinformativas/2026-medilegales-notasinformativas/','medicamento_ilegal')]
def fetch(url):
...
def iso_date(raw):
 ...
 mo=months.get(m.group(2).lower());return f'{m.group(3)}-{mo:02d}-{int(m.group(1)):02d}' if mo else None
def parse(page,name,cat):
 items=[];blocks=re.findall(r'<article\b.*?</article>',page,re.I|re.S) or re.split(r'(?=<h[23]\b)',page,flags=re.I)[1:]
 for b in blocks:
  link=re.search(r'<a\b[^>]*href=["\'](https://www\.aemps\.gob\.es/[^"\']+)["\'][^>]*>(.*?)</a>',b,re.I|re.S)
  if not link:continue
  title=text(link.group(2));body=text(b);date=iso_date(body)
  if len(title)<8 or not date:continue
...
def main():
 ...
 all_items=[];health=[];failed=set()
 for name,url,cat in SOURCES:
  try:
   found=parse(fetch(url),name,cat);had_old=any(x.get('categoria')==cat for x in old.get('items',[]))
   if not found and had_old:health.append({'fuente':name,'estado':'error','detalle':'parser_sin_resultados'});failed.add(cat);continue
   all_items.extend(found);health.append({'fuente':name,'estado':'ok','detectadas':len(found)})
  except Exception as e:health.append({'fuente':name,'estado':'error','detalle':type(e).__name__});failed.add(cat)
 ok=sum(x['estado']=='ok' for x in health)
 if ok==0:print('AEMPS: ninguna fuente pudo validarse; se conservan datos anteriores');return
 # Una fuente fallida conserva solo sus propios registros previos, sin contaminar categorías revisadas correctamente.
